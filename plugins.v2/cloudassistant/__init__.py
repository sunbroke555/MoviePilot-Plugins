...
class CloudAssistant(_PluginBase):
    ...
    def __handle_file(self, event_path: str, mon_path: str):
        ...
        try:
            ...
            with lock:
                ...
                path_115 = monitor_dir.get("115_path")
    ...
    def __check_file_exists(mount_file, retries=3, wait_time=20):
        for attempt in range(retries):
            if Path(mount_file).exists():
                logger.info(f"挂载目录文件 {mount_file} 存在。")
                return True
            else:
                logger.info(f"挂载目录文件 {mount_file} 不存在，尝试 {attempt + 1}/{retries}。")
                time.sleep(wait_time)
        logger.info(f"挂载目录文件 {mount_file} 经过 {retries} 次检查仍然不存在，不创建。")
        return False
    ...
    def __refresh_emby(self, transferinfo):
        ...
        for emby_name, emby_server in emby_servers.items():
            logger.info(f"开始通知媒体服务器 {emby_name} 刷新媒体库")
            self._EMBY_APIKEY = emby_server.config.config.get("apikey")
            self._EMBY_HOST = emby_server.config.config.get("host")
            ...
            if transferinfo.type == "电影":
                movies = emby_server.instance.get_movies(title=transferinfo.title, year=transferinfo.year)
                if not movies:
                    logger.error(f"{emby_name} 中没有找到{transferinfo.title} ({transferinfo.year})")
                    break
                for movie in movies:
                    self.__refresh_emby_library_by_id(item_id=movie.item_id)
                    logger.info(f"{emby_name} 已通知刷新电影：{movie.title} ({movie.year}) item_id:{movie.item_id}")
            else:
                ...
                # 验证tmdbid是否相同
                item_info = emby_server.instance.get_iteminfo(item_id)
                if item_info:
                    if transferinfo.tmdbid and item_info.tmdbid:
                        if str(transferinfo.tmdbid) != str(item_info.tmdbid):
                            logger.error(
                                f"{emby_name} 中{transferinfo.title} ({transferinfo.year})的tmdbId与入库记录不一致")
                            break
                ...
                if not episode_item_id or episode_item_id is None:
                    logger.error(
                        f"{emby_name} 中没有找到{transferinfo.title} ({transferinfo.year}) {transferinfo.seasons}{transferinfo.episodes}")
                    break

                self.__refresh_emby_library_by_id(item_id=episode_item_id)
                logger.info(
                    f"{emby_name} 已通知刷新电视剧：{transferinfo.title} ({transferinfo.year}) {transferinfo.seasons}{transferinfo.episodes} item_id:{episode_item_id}")
