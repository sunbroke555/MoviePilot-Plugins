...
class CloudCompanion(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "cloudcompanion_"
    # 加载顺序
    plugin_order = 26
    # 可使用的用户级别
    auth_level = 1
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            if self._115_cookie:
                ...
                logger.error("115_cookie 未配置，请检查配置")
                return
        ...
        if self._enabled or self._onlyonce:
            ...
            # 运行一次定时服务
            if self._onlyonce:
                logger.info("云盘伴侣全量执行服务启动，立即运行一次")
    ...
    def scan(self, event: Event = None):
        ...
        if event:
            ...
            if not event_data or event_data.get("action") != "cloud_companion":
                return
            logger.info("收到命令，开始云盘伴侣生成 ...")
    ...
    def __softlink(self, source_file, target_file):
        """
        创建软链接
        """
        # 文件
        if Path(target_file).exists():
            if os.path.islink(target_file):
                current_target = os.readlink(target_file)
                if str(current_target) != str(target_file):
                    subprocess.run(['ln', '-sf', source_file, target_file])
                    logger.info(f"修正软连接 {target_file} -> {source_file}")
                else:
                    logger.info(f"软连接 {target_file} 已存在")
                return
            else:
